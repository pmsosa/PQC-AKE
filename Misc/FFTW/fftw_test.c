...
#include <inttypes.h>   //Print uint16_t
...
int main(void)
{
	...
	{
		if (i < n){
            a[i] = 1;//round(rand()/(RAND_MAX+1.)*10+1);
            b[i] = 1;//round(rand()/(RAND_MAX+1.)*10+1);
        }
        else{
            a[i] = 0.;
            b[i] = 0.;
    ...
    printf("Actual Polynomial Multiplication!\n");
    polymult(r,a,b,n);
}
